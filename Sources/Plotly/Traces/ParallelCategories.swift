...
public struct ParallelCategories: Trace, DomainSubplot {
    ...
    /// `count`, `probability`, `category`, `categorycount`, `colorcount` and `bandcolorcount`. Anything
    ...
    public struct ShapedMarkerLine: Encodable {
        ...
        /// link https://plotly.com/javascript/plotlyjs-events/#event-data. Additionally, every attributes
        /// that can be specified per-point (the ones that are `arrayOk: true`) are available. variables
        /// `count` and `probability`. Anything contained in tag `<extra>` is displayed in the secondary
